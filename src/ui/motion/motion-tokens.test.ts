...
describe("motion tokens", () => {
  it("keeps visual timings inside the premium interaction budget", () => {
    ...
    expect(motionTokens.duration.focusFast).toBeLessThanOrEqual(140);
    expect(motionTokens.duration.standard).toBeGreaterThanOrEqual(170);
    expect(motionTokens.duration.standard).toBeLessThanOrEqual(190);
    expect(motionTokens.duration.backgroundCrossfade).toBeGreaterThanOrEqual(
      260,
    );
    expect(motionTokens.duration.backgroundCrossfade).toBeLessThanOrEqual(340);
  });
